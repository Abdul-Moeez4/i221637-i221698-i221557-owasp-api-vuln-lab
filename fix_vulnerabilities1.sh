...
# -----------------------------------------
# PATHS UPDATED FOR REPO STRUCTURE
# -----------------------------------------
SECURITY_CONFIG="owasp-api-vuln-lab/src/main/java/edu/nu/owaspapivulnlab/config/SecurityConfig.java"
USER_CONTROLLER="owasp-api-vuln-lab/src/main/java/edu/nu/owaspapivulnlab/web/UserController.java"
DTO_DIR="owasp-api-vuln-lab/src/main/java/edu/nu/owaspapivulnlab/dto"
RATE_LIMIT_CONFIG="owasp-api-vuln-lab/src/main/java/edu/nu/owaspapivulnlab/config/RateLimitConfig.java"
JWT_SERVICE="owasp-api-vuln-lab/src/main/java/edu/nu/owaspapivulnlab/service/JwtService.java"
...
if [ -f "$SECURITY_CONFIG" ]; then
  ...
  echo "⚠️ SecurityConfig.java not found."
fi
...
EOF
  ...
  echo "⚠️ UserController.java not found."
fi
...
if [ -f "$USER_CONTROLLER" ] && ! grep -q 'AppUserRequestDTO' "$USER_CONTROLLER"; then
  cat << 'EOF' >> "$USER_CONTROLLER"
...
EOF
  ...
  echo "✅ Request DTOs already defined or UserController missing."
fi
...
if [ -f "$JWT_SERVICE" ]; then
  ...
  echo "⚠️ JwtService.java not found."
fi
